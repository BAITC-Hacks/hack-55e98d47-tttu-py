...
def _matched_view(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    count = len(cards)
    suffix = "Показан единственный найденный вариант." if count == 1 else (
        "Показаны оба найденных варианта." if count == 2 else "Показаны три лучших варианта."
    )
    return {
        "kind": "matched",
...
def _result_view(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if status == "category_not_found":
        return {
            ...
            "message": result.get("message") or "В этом городе нет подрядчиков выбранной категории.",
        }
    if status == "no_eligible_candidates":
        ...
        return {
            ...
            "message": result.get("message") or "Подрядчики есть, но ни один не прошёл условия заказа.",
            "reasons": reasons,
        }
    return {
        "kind": "empty",

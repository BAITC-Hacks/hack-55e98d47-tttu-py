...
Административная подготовка каталога пока не является полноценным lifecycle:
нет HTTP-аутентификации, checksum/version metadata, activate/rollback workflow.
Команды `validate` и `stage` предназначены только доверенному локальному
оператору.

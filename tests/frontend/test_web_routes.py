...
def test_matched_renders_at_most_three_cards_and_markers():
    ...
    response = make_client(StubService({"status": "matched", "recommendations": items})).post("/", data=valid_form())
    assert response.status_code == 200
...
def test_category_not_found_is_distinct():
    result = {"status": "category_not_found", "message": "В Астане нет подрядчиков категории «Флорист»."}
    response = make_client(StubService(result)).post("/", data=valid_form())
...
def test_empty_matched_response_never_leaves_blank_area():
    response = make_client(StubService({"status": "matched", "recommendations": []})).post("/", data=valid_form())
    assert "Рекомендации не получены" in response.text


def test_validation_and_backend_errors_are_visible_and_preserve_form():
    validation = make_client(StubService(error=ValueError("Бюджет некорректен"))).post("/", data=valid_form())
    assert validation.status_code == 422
    assert "Бюджет некорректен" in validation.text
    assert 'value="1000000"' in validation.text

    backend = make_client(StubService(error=RuntimeError("secret backend detail"))).post("/", data=valid_form())
